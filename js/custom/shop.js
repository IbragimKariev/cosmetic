(function ($) {
    ...
    var loadCart = function () {
        ...
        for (var key in cart) {
            ...
            $(selector).hasClass("single-product-item") ?
                $(selector).find(".to-cart-btn-text").html("Added") :
                $(selector).find(".to-cart-btn").html("Added")
        }
    };

    // Для кнопки "Add to cart" в shop.html и single-product.html
            ...
            $(".to-cart-btn").on("click", function (event) {
                ...
                if (addProductToCart(prod, dataId)){
                    productElement.hasClass("single-product-item") ?
                        productElement.find(".to-cart-btn-text").html("Added") :
                        productElement.find(".to-cart-btn").html("Added");
                    borderIfColorsSelected();
                } else {
                    productElement.hasClass("single-product-item") ?
                        productElement.find(".to-cart-btn-text").html("Add to cart") :
                        productElement.find(".to-cart-btn").html("Add to cart")
                }
    ...
    var loadWishList = function () {
        ...
        for (var key in wishList) {
            ...
            $(selector).hasClass("single-product-item") ?
                ...
                $(selector).find(".to-wishlist-btn").html("Added")
        }
    };
        ...
        $(".to-wishlist-btn").on("click", function (event) {
            ...
            if (!wishList[dataId]) {
                ...
                productElement.hasClass("single-product-item") ?
                    ...
                    productElement.find(".to-wishlist-btn").html("Added")
            } else {
    ...
    var renderProductsInWishlist = function () {
        ...
        for (var key in wishList) {
            ...
            productStatus = ifProductExistInCart(key) ? "Added" : "Add to cart";
    ...
    var onAddToCartFromWishlistListener = function () {
        $(".to-cart-from-wishlist-btn").on("click", function (event) {
            ...
            if (cart.hasOwnProperty(productId)) {
                ...
                $(selector).find(".to-cart-from-wishlist-btn").html("Add to cart");
            } else {
                cart = Object.assign(cart, product);
                $(selector).find(".to-cart-from-wishlist-btn").html("Added");
            }
